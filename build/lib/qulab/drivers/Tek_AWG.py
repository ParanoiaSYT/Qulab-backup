...
class Driver(BaseDriver):
    ...
    quants = [
        ...
        QReal('Phase', unit='deg', ch = 1,
          set_cmd = 'SOUR%(ch)d:PHAS:ADJust %(value)f',
          get_cmd = 'SOUR%(ch)d:PHAS:ADJust?'),
...
# 简化波形生成函数调用
    # def AWGrun(self, AWG = 'AWG', array = array, ch=0, width=0, sr = 0):
    #     '''从文件或序列快速产生波形'''
    #     self.setValue('remove_waveform', 'AWG')
    #     self.setValue('create_waveform', 'AWG', sr)
    #     self.setValue('update_waveform', array = array, name = 'AWG', size = int(width*sr))
    #     self.setValue('use_waveform', 'AWG', ch = ch)
    #     Amplitude=self.getValue('Amplitude',ch = ch)
    #     self.setValue('Amplitude', Amplitude, ch = ch)
    #     self.setValue('output_on', ch = ch)
        # self.setValue('run')
        ...
        # self.config['output_on'][ch]['value']='Run'

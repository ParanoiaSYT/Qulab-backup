...
def imshow(fig, x, y, z, xlabel='x (a.u.)', ylabel='y (a.u.)', zlabel='z (a.u.)'):
    ...
    if isinstance(x, np.ndarray) and isinstance(y, np.ndarray):
        map = ax.imshow(z, extent=(min(x), max(x), min(y), max(y)),
...
def imshow_2(fig, xmin,xmax, y, z, xlabel='x (a.u.)', ylabel='y (a.u.)', zlabel='z (a.u.)'):
    ax = fig.add_subplot(111)

    if isinstance(x, np.ndarray) and isinstance(y, np.ndarray):
        ax.imshow(z, extent=(xmin, xmax, min(y), max(y)),
                     aspect='auto', origin='lower', interpolation='nearest')
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
    else:
